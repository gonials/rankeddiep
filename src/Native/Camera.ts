...
import MinimapIndicator from "../Entity/Misc/MinimapIndicator";
...
import { removeFast, randomFrom, sendToWebhook } from "../util";

import { compileCreation, compileUpdate } from "./UpcreateCompiler";
import { maxPlayerLevel, host } from "../config";
...
export class CameraEntity extends Entity {
    ...
    public spectatee: ObjectEntity | CameraEntity | null = null;
    
    /** Cameras spectating this camera (deathscreen). */
    public spectators: CameraEntity[] = [];
...
export default class ClientCamera extends CameraEntity {
    ...
    public constructor(game: GameServer, client: Client) {
        ...
        this.cameraData.values.respawnLevel = this.cameraData.values.level = 45;
        this.cameraData.values.score = 1;
    ...
    private updateView(tick: number) {
        ...
        const width = (1920 / fov) / 1.75;
        const height = (1080 / fov) / 1.75;
        ...
        for (let id = 0; id < this.game.entities.indicatorEntities.length; ++id) {
            const entity = this.game.entities.inner[this.game.entities.indicatorEntities[id]] as MinimapIndicator;
            
            if (!entitiesInRange.includes(entity) && entity.isVisible(this)) entitiesInRange.push(entity);
        }
    ...
    public findSpectateTarget(): boolean {
        const team = this.relationsData.values.team;
        const isTeam = TeamEntity.isTeam(team);
        
        let teammates: TankBody[] = [];
        
        if (isTeam) teammates = this.game.arena.getTeamPlayers(team);

        const targets = teammates.length > 0 ? teammates : this.game.arena.getAlivePlayers();
        const validTargets: TankBody[] = [];

        for (const target of targets) {
            if (Entity.exists(target) && target.healthData.values.health > 0) validTargets.push(target);
        }

        if (!validTargets.length) return false;

        const spectatee = this.spectatee?.cameraData?.values.player || this.spectatee; // If one already exists and switching
        const indexOf = validTargets.indexOf(spectatee as TankBody); // Just to make typescript shut up for once

        if (indexOf === -1) {
            const target = randomFrom(validTargets); // Pick a random target if not
            this.setSpectateTarget(target);
            return true;
        } else {
            const idx = (indexOf + 1) % validTargets.length; // cycle through em
    ...
    public setSpectateTarget(target: ObjectEntity | CameraEntity | null) {
        const oldTarget = this.spectatee;
        if (oldTarget && oldTarget.cameraData) {
            const spectators = (oldTarget as CameraEntity).spectators;
            if (spectators.includes(this)) {
                removeFast(spectators, spectators.indexOf(this));
                this.game.sendSpectatorList();
            }
        }

        if (TankBody.isTank(target)) {
            const camera = target.cameraEntity;
            this.spectatee = camera;
            camera.spectators.push(this);
            this.game.sendSpectatorList();
        } else {
            this.spectatee = target;
        }
    }
    ...
    public tick(tick: number) {
        ...
        const spectatee = this.spectatee;
        if (!Entity.exists(this.cameraData.values.player)) {
            if (Entity.exists(spectatee)) {
                const isCamera = !!spectatee.cameraData;
                
                const pos = { x: 0, y: 0 };

                if (isCamera) {
                    pos.x = (spectatee as CameraEntity).cameraData.values.cameraX;
                    pos.y = (spectatee as CameraEntity).cameraData.values.cameraY;
                    this.cameraData.FOV = (spectatee as CameraEntity).cameraData.values.FOV;
                    
                    if (!Entity.exists((spectatee as CameraEntity).cameraData.values.player)) this.setSpectateTarget(null);
                } else if (ObjectEntity.isObject(spectatee)) {
                    pos.x = spectatee.positionData.values.x;
                    pos.y = spectatee.positionData.values.y;
                }
