...
import { Color, ArenaFlags, CameraFlags, StyleFlags, Tank, ValidScoreboardIndex } from "../Const/Enums";
...
import { AccessLevel, countdownDuration, bossSpawningInterval, factorySpawnChance, scoreboardUpdateInterval } from "../config";
...
export default class ArenaEntity extends Entity implements TeamGroupEntity {
    ...
    protected updateScoreboard(scoreboardPlayers: TankBody[]) {
        ...
        for (let i: ValidScoreboardIndex = 0; i < scoreboardCount; i = (i + 1) as ValidScoreboardIndex) {
            ...
            this.arenaData.values.scoreboardNames[i] = this.game.anonymousMode ? "" : player.nameData.values.name;
    ...
    public getAlivePlayers() {
        ...
        for (const client of this.game.clients) {
            ...
            if (Entity.exists(entity) && TankBody.isTank(entity) && !entity.definition.flags.isSpectator) players.push(entity);
        }
        return players;
    }
    ...
    public isTankUpgradeAllowed(client: Client, tankId: Tank): boolean {
        return true; // Override in gamemode files
    }

    public canResetStats(client: Client): boolean {
        const player = client.camera?.cameraData.values.player;
        if (!TankBody.isTank(player)) return false;
        
        if (player.isInvulnerable || player.styleData.values.flags & StyleFlags.isFlashing) return true;

        client.notify("You must be invulnerable in order to reset stats", 0xFF0000, 10_000, "stat_reset_fail");
        return false;
    }
    
    public canSuicide(client: Client): boolean {
        const player = client.camera?.cameraData.values.player;
        if (!ObjectEntity.isObject(player) || player.deletionAnimation) return false;
        
        if (
            client.accessLevel >= AccessLevel.BetaAccess ||
            (this.game.arena.arenaData.values.flags & ArenaFlags.canUseCheats)
        ) return true;
        
        return false;
    }

    public switchTeam(client: Client): void {
        return; // Override in gamemode files
    }
