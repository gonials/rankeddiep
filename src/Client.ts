...
import { bannedClients, getDiscordUserId, isDiscordVerified } from ".";
...
export class ClientInputs extends Inputs {
    ...
    public unsafeMouseX: number = 0;
    public unsafeMouseY: number = 0;
...
export default class Client {
    ...
    public discordData: DiscordData = { username: "unset", id: "unset" };
    ...
    public _proxyTarget: Client | null = null;
    ...
    public async verifyClient() {
        ...
        if (!config.enableVerification) return this.acceptClient(); // no auth, verify instantly

        const userData = ws.getUserData();
        try {
            const session = userData.session;
            const discordData = await isDiscordVerified(session);
            const accessLevel = discordData.accessLevel as config.AccessLevel ?? config.AccessLevel.NoAccess;
            if (accessLevel === config.AccessLevel.NoAccess) {
                ws.close();
                return;
            }

            if (this.terminated || !ws) return;

            this.discordData.id = discordData.userId;
            this.discordData.username = discordData.username;
    ...
    public acceptClient() {
        ...
        /*this.game.broadcastMessage(
            `${this.discordData.username} connected`
        )*/
    }
    ...
    public send(data: Uint8Array) {
        const ws = this.ws;
        if (!ws) return; // throw new Error("Can't write to a closed websocket - shouldn't be referencing a closed client");
    ...
    public terminate() {
        ...
        if (Entity.exists(this.camera)) {
            if (Entity.exists(this.camera.cameraData.values.player)) {
                const username = this.discordData.username || "unknown user";
                this.game.broadcastMessage(
    ...
    private handleIncoming(header: number, data: Uint8Array): void {
        ...
        switch (header) {
            case ServerBound.Input: {
                ...
                this.inputs.unsafeMouseX = mouseX;
                this.inputs.unsafeMouseY = mouseY;
                ...
                if ((flags & InputFlags.rightclick) && !(previousFlags & InputFlags.rightclick) && (player.definition.flags.isSpectator)) {
                ...
                if ((flags & InputFlags.suicide) && this.game.arena.canSuicide(this)) {
                    player.destroy();
            ...
            case ServerBound.TankUpgrade: {
                ...
                if (!definition || !definition.upgrades.includes(tankId) || !tankDefinition || tankDefinition.levelRequirement > camera.cameraData.values.level || !this.game.arena.isTankUpgradeAllowed(this, tankId)) return;
            ...
            case ServerBound.SwitchTeam: {
                this.game.arena.switchTeam(this);

                return;
            }

            case ServerBound.Spectate: {
                if (Entity.exists(camera.cameraData.values.player)) return;

                camera.findSpectateTarget();

                return;
            }

            case ServerBound.ResetStats: {
                if (!this.game.arena.canResetStats(this)) return;

                const player = camera.cameraData.values.player;
                if (!Entity.exists(player) || !TankBody.isTank(player)) return;
                this.resetStats();
                this.notify("Stats have been reset", 0x43FFF9, 10_000, "stat_reset");

                return;
            }
            
            case ServerBound.DistressSignal: {
                const player = camera.cameraData.values.player;
                
                if (!Entity.exists(player) || !TankBody.isTank(player)) return;

                player.minimapIndicator?.distressSignal();
                return;
            }
            
            case ServerBound.Zoom: {
                if (this.accessLevel < config.AccessLevel.kReserved) return;

                const tank = camera.cameraData.values.player;
                if (!Entity.exists(tank) || !TankBody.isTank(tank)) return;

                if (tank.definition.flags.isSpectator) { // Paranoia
                    const zoomAddition = r.u8() === 0 ? 0.02 : -0.02;


                    /*this.game.broadcast()
                    .u8(ClientBound.Notification)
                    .stringNT(`${tank.nameData.values.name || "an unnamed tank"} is using server-side FOV`)
                    .u32(0x000000)
                    .float(10000)
                    .stringNT("fov").send();
                    */
                    camera.cameraData.FOV = util.constrain(camera.cameraData.values.FOV + zoomAddition, 0.05, 5);
                }
                return;
            }
    ...
    public resetStats() {
        ...
        for (let i = Stat.MovementSpeed; i <= Stat.HealthRegen; ++i) {
            camera.cameraData.statsAvailable += camera.cameraData.values.statLevels.values[i];
            camera.cameraData.statLevels[i] = 0;

            this.write().u8(ClientBound.ResetStatQueue).send(); // Reset clientside stat queue
        }
    }
    ...
    public possess(ai: AI) {
        ...
        // this.camera.relationsData.team = ai.owner.relationsData.values.team;
    ...
    public tick(tick: number) {
        ...
        if (tick >= this.lastPingTick + 60 * 10 * config.tps) {
            return this.terminate();
        }
    }
