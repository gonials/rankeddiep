...
export default class ObjectEntity extends Entity {
    ...
    public static isColliding(objA: ObjectEntity, objB: ObjectEntity): boolean {
        ...
        if (
            (relationsA.owner?.positionData && positionB.flags & PositionFlags.ignoresProjectiles) ||
            (relationsB.owner?.positionData && positionA.flags & PositionFlags.ignoresProjectiles)
        ) {
            return false;
        }
