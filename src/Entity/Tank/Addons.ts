...
class HeavyBaseAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        const base = this.createGuard(6, 1.3, 0, 0);
        base.physicsData.values.flags &= ~PositionFlags.absoluteRotation;
    }
}
...
class LauncherAddon extends Addon {
    public launcher: ObjectEntity;

    public constructor(owner: BarrelBase) {
        super(owner);

        const launcher = this.launcher = new ObjectEntity(this.game);
...
/** Dual Shot's addon */
class DualShotAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);
...
/** Pellet Shot's addon */
class PelletShotAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);
...
/** Glider's addon. */
class SniperLauncherAddon extends LauncherAddon {
    public constructor(owner: BarrelBase) {
        super(owner);

        this.launcher.positionData.values.angle = Math.PI;
    }
}

/** The thing underneath Rocketeer and Twister addon. */
class HeavyLauncherAddon extends Addon {
    public launcher: ObjectEntity;
 ...
 */
export const AddonById: Record<addonId, typeof Addon> = {
    spike: SpikeAddon,
    dombase: DomBaseAddon,
    launcher: LauncherAddon,
    sniperLauncher: SniperLauncherAddon,
    heavyLauncher: HeavyLauncherAddon,
    ...
    spiesk: SpieskAddon,
    heavy: HeavyBaseAddon,
    shotgun: ShotgunAddon,
    dualshot: DualShotAddon,
    pelletshot: PelletShotAddon
}