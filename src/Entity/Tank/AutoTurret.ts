...
export default class AutoTurret extends ObjectEntity {
    ...
    public constructor(owner: BarrelBase, barrelDefinition: BarrelDefinition[] | BarrelDefinition = AutoTurretDefinition, baseSize: number = 25) {
        ...
        for (let i = 0; i < barrelDefinition.length; ++i) {
            const def = barrelDefinition[i];
            const turret = new Barrel(this, def);
            turret.physicsData.values.flags |= PhysicsFlags.doChildrenCollision;
            this.turret.push(turret);
        }
    }
        ...
        if (useAI) {
            if (this.ai.state === AIState.idle) {
                ...
                for (let i = 0; i < this.turret.length; ++i) {
                    this.turret[i].attemptingShot = false;
                }
            } else {
                // Uh. Yeah
                const {x, y} = this.getWorldPosition();
