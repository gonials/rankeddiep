...
import { TeamEntity } from "../Misc/TeamEntity";
...
import MouseIndicator from "../Misc/MouseIndicator";
import VisionIndicator from "../Misc/VisionIndicator";
...
export default class TankBody extends LivingEntity implements BarrelBase {
    ...
    /** Minimap dot of the tank. */
    public minimapIndicator: MinimapIndicator | null = null;
    ...
    public constructor(game: GameServer, camera: CameraEntity, inputs: Inputs) {
        ...
        if (this.game.playersOnMap) {
            this.minimapIndicator = new MinimapIndicator(this);
        }

        new MouseIndicator(this);
        new VisionIndicator(this);
    }
    ...
    public onDeath(killer: LivingEntity) {
        ...
        if (!client) return camera.delete();

        if (camera.cameraData.player !== this) return;

        camera.cameraData.killedBy = (killer.nameData && killer.nameData.values.name) || "";
        
        const _camera = camera as ClientCamera; // Only client-cameras have a client, so just force that

        if (!TeamEntity.isTeam(this.relationsData.values.team)) {
            _camera.setSpectateTarget(killer);
        } else {
            const success = _camera.findSpectateTarget();
            if (!success) _camera.spectatee = killer; // Spectate killer instead if there are no valid teammates
        }
    }
    ...
    public destroy(animate=true) {
        ...
        if (!animate && Entity.exists(this.cameraEntity)) {
            if (this.cameraEntity.cameraData.player === this) {
                ...
                this.cameraEntity.cameraData.respawnLevel = maxPlayerLevel;
            }
